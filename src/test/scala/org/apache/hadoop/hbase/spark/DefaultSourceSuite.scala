...
BeforeAndAfterEach with BeforeAndAfterAll {
  ...
  override def beforeAll() {
    ...
    try
      ...
      case e: Exception => println(" - no table " + t1TableName + " found")
    }
    try
      TEST_UTIL.deleteTable(TableName.valueOf(t2TableName))
    catch {
      case e: Exception => println(" - no table " + t2TableName + " found")
    }
    println(" - creating table " + t1TableName)
    TEST_UTIL.createTable(TableName.valueOf(t1TableName), Bytes.toBytes(columnFamily))
    println(" - created table")
    println(" - creating table " + t2TableName)
