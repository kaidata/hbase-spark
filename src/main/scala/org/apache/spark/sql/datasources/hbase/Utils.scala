  ...
  def toBytes(input: Any, field: Field): Array[Byte] = {
    if (field.schema.isDefined) {
      ...
      input match {
        ...
        case str: StringType => Bytes.toBytes(str.asInstanceOf[String])
        case str: StringType => Bytes.toBytes(str.asInstanceOf[String])
        // TODO: add more data type support
        case _ => {
          //          LOG.error(s"unsupported data type ${field}, ${input}")

          if (input == null) {
            return null
          } else {
            Bytes.toBytes(input.asInstanceOf[DataType].toString)
          }
        }
      }
    }
